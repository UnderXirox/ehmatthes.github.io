...
Now we'll replace all of the bold lines with two blocks, {{ title }} and {{ content }}:
...
Copy md files here.
Link to them; change links to .html.
Create _layouts dir, and _layout from index.html.
Stylesheet links.
Preview locally.
Commit and push.

create the actual blog
  develop locally, to see more subtle markdown issues ie large code blocks
...
- need sudo?
    - or add ruby bin to path
- (done) create index page on github
- create vm
- fetch branch
- work locally
    - install ruby, set up jekyll
    - jekyll server
    - customize theme for sub-pages
- push live
    - create my user pages site, and probably _posts dir
- cleaning up
    - border on pages screenshot?
    - would help to have shift-prtscrn set up
